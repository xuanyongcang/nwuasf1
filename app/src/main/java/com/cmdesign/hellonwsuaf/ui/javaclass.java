...
//				Toast.makeText(ScheduleActivity.this, "get schedule success", Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        roundProBar = (ProgressBar) findViewById(R.id.schedule_progressbar2);
        scheduleView = (ScheduleView) this.findViewById(R.id.scheduleView2);
        result= pref2.getString("schedule", "");
        if(result!=""){
        toShow();
    ...
    private void toShow() {
        ...
        if (!isRemember) {
            Intent intent = new Intent(javaclass.this, Loginn2Activity.class);
            Toast.makeText(javaclass.this, "载入失败请先登录", Toast.LENGTH_SHORT).show();
            javaclass.this.startActivity(intent);
        } else {
            Toast.makeText(javaclass.this, "载入成功", Toast.LENGTH_LONG).show();
...
//        map.put("YearTermNO", "14");
     ...
     * 显示课程表
     ****************************/
    private void toShow2() {
        classList = new ArrayList<ClassInfo>();
        String text2 = null;
        if (result == null) {
            Intent intent = new Intent(javaclass.this, Loginn2Activity.class);
            Toast.makeText(javaclass.this, "登陆失败请检查学号密码和验证码是否正确", Toast.LENGTH_SHORT).show();
            javaclass.this.startActivity(intent);
        } else {
            System.out.println(result);
            Log.e("TAG",result);
            ...
            editor.commit();
            Log.d("tag",result);
            Toast.makeText(javaclass.this, "登陆成功", Toast.LENGTH_SHORT).show();
            Document doc = Jsoup.parse(result, "UTF-8");
            Elements tables = doc.select("table[id=print]");
            if(tables==null)Toast.makeText(javaclass.this, "登陆失败请检查学号密码和验证码是否正确", Toast.LENGTH_SHORT).show();;
            Elements trs = tables.select("tr");
            Elements tds = trs.select("td[class=c]");//td[rowspan=2]
            int num=1;
            int suanshu=1;
